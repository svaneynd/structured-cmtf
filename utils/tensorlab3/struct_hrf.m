function [x,state] = struct_hrf_v2(z,task,t)
%STRUCT_HRF Haemodynamic response function.
%   [x,state] = struct_hrf_v2(z,[],t) computes the haemodynamical response 
...
%   REMARK: 
%   This version allows scaling relative scaling of the two gamma functions
%   by including an extra parameters.
%   A straightforward extension is to include more gamma pdfs
...
%     x = g1 - z(5)*g2;
    x = ((z(3)^z(1))*(u.^(z(1)-1)).*exp(-z(3)*u))/gamma(z(1)) - z(5)*((z(4)^z(2))*(u.^(z(2)-1)).*exp(-z(4)*u))/gamma(z(2));
    
    % compute the partial derivatives
    der1 = g1.*(log(z(3)) + log(u) - psi(0,z(1)));
    der2 = -z(5)*g2.*(log(z(4)) + log(u) - psi(0,z(2)));
    der3 = g1.*(z(1)/z(3) - u);
    der4 = -z(5)*g2.*(z(2)/z(4) - u);
    der5 = -g2;
    state.deriv = [ der1(:) , der2(:) , der3(:) , der4(:) , der5(:) ];
