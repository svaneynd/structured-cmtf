...
function grad = grad(z)
    ...
    for i = find(options.Weights(:).' ~= 0)
        ...
        JHF = options.Weights(i) * model.factorizations{i}.grad(i,tmp);
        
        % Contract the factor matrices into variables.
        grad = derivcontract(i,grad,JHF);
%         if i == 1
...
% Expand the variables into factors.
    ...
    for i = find(options.Weights(:).' ~= 0)
        ...
        tmpb = getfactors(i,x);
        JHJx = options.Weights(i) * model.factorizations{i}.JHJx(i,tmpa,tmpb);
        
        % Contract the factor matrices into variables.
        y = derivcontract(i,y,JHJx);
%         if i == 1
